...
type FitType = 'slim' | 'regular' | 'oversized';
type LengthType = 'cropped' | 'normal' | 'long';

function clamp(n: number, min: number, max: number) {
  return Math.min(Math.max(n, min), max);
}

function smoothRect(prev: ClothingRect | null, next: ClothingRect, alpha = 0.25): ClothingRect {
  if (!prev) return next;
  return {
    left: prev.left * (1 - alpha) + next.left * alpha,
    top: prev.top * (1 - alpha) + next.top * alpha,
    width: prev.width * (1 - alpha) + next.width * alpha,
    height: prev.height * (1 - alpha) + next.height * alpha,
  };
}

function inferFitType(product: Product): FitType {
  const source = `${product.name ?? ''} ${product.category ?? ''} ${(product as any).fitType ?? ''}`.toLowerCase();
  if (source.includes('slim') || source.includes('skinny') || source.includes('fitted')) return 'slim';
  if (source.includes('oversized') || source.includes('relaxed') || source.includes('loose')) return 'oversized';
  return 'regular';
}

function inferLengthType(product: Product): LengthType {
  const source = `${product.name ?? ''} ${product.category ?? ''} ${(product as any).lengthType ?? ''}`.toLowerCase();
  if (source.includes('crop') || source.includes('cropped')) return 'cropped';
  if (source.includes('long') || source.includes('tunic') || source.includes('maxi')) return 'long';
  return 'normal';
}

function overlayFitMultiplier(product: Product) {
  const fit = inferFitType(product);
  if (fit === 'slim') return 0.94;
  if (fit === 'oversized') return 1.12;
  return 1;
}

function overlayLengthMultiplier(product: Product) {
  const length = inferLengthType(product);
  if (length === 'cropped') return 0.82;
  if (length === 'long') return 1.14;
  return 1;
}
...
function parseMeasurementNumber(value: unknown): number | null {
  ...
  const match = value.replace(/cm/gi, '').match(/\d+(?:\.\d+)?/);
  if (!match) return null;
  return Number(match[0]);
}

function getProductFitRecommendation(product: Product, body: { chestIn: number; hipWidthIn: number; inseamIn: number }, fallbackSize: string) {
  const measurements = (product as any).measurements;
  const cat = product.category.toLowerCase();
  const fit = inferFitType(product);
  const isLower = cat.includes('bottom') || cat.includes('jean') || cat.includes('pant') || cat.includes('skirt');
  const target = isLower ? body.hipWidthIn : body.chestIn;
  const requiredEase = fit === 'slim' ? 1 : fit === 'oversized' ? 5 : 3;

  if (!measurements || typeof measurements !== 'object') {
    return {
      size: fallbackSize,
      summary: `${fit} visual estimate • add a product size chart for stronger comparison`,
      method: 'Body estimate',
    };
  }

  const looksLikeSize = (size: string) => /^(xxs|xs|s|m|l|xl|xxl|xxxl|\d{2,3})$/i.test(size.trim());
  const candidates = Object.entries(measurements)
    .filter(([size]) => looksLikeSize(size))
    .map(([size, raw]) => ({ size, garment: parseMeasurementNumber(raw) }))
    .filter((x): x is { size: string; garment: number } => x.garment !== null)
    .sort((a, b) => a.garment - b.garment);

  if (!candidates.length) {
    return {
      size: fallbackSize,
      summary: `${fit} estimate • size guide format not numeric enough to compare`,
      method: 'Body estimate',
    };
  }

  const best = candidates.find(c => c.garment - target >= requiredEase) ?? candidates[candidates.length - 1];
  const ease = best.garment - target;
  const fitLabel = ease < 1 ? 'tight' : ease < 3 ? 'slim fit' : ease < 5 ? 'regular fit' : 'roomy fit';

  return {
    size: best.size,
    summary: `${fitLabel} by size guide • ${Math.round(ease)}\" ease`,
    method: 'Body + product chart',
  };
}
...
export function ARViewer({ product, onClose, activeImage }: ARViewerProps) {
  ...
  function calcOverlays(lms: PoseLandmark[], cw: number, ch: number) {
    ...
    const fitMul = overlayFitMultiplier(product);
    const lenMul = overlayLengthMultiplier(product);

    // More stable Y anchoring: shoulder width is usually less jumpy than hip Y.
    const upperW = shW * 2.5 * fitMul;
    const upperH = Math.max(torsoH * 1.45, shW * 2.15) * lenMul;
    const upper: ClothingRect = {
      left: shCX - upperW / 2,
      top: shCY - shW * 0.35,
      width: upperW,
      height: upperH,
    ...
    const lowW = shW * 1.75 * fitMul;
    const lowerTop = hipCY - torsoH * 0.10;
    const lower: ClothingRect = {
      left: shCX - lowW / 2,
      top: lowerTop,
      width: lowW,
      height: Math.max(ankleY - lowerTop + torsoH * 0.05, torsoH * 1.25),
    };

    const fullW = shW * 2.45 * fitMul;
    const full: ClothingRect = {
      left: shCX - fullW / 2,
      top: shCY - shW * 0.32,
      width: fullW,
      height: Math.max(kneeY - shCY + torsoH * 0.38, torsoH * 2.1) * lenMul,
    };

    let shoe:ClothingRect|null=null;
    if (la&&ra&&(la.visibility??0)>0.3) {
      const ankX=(mx(la.x)+mx(ra.x))/2,ankY2=(my(la.y)+my(ra.y))/2;
      const shoeW=clamp(shW*1.2,120,240);
      shoe={left:ankX-shoeW/2,top:ankY2-shoeW*0.10,width:shoeW,height:shoeW*0.60};
    }

    if (region==='upper' || region==='unknown') {
      setUpperRect(prev=>smoothRect(prev, upper));
      setLowerRect(null); setFullRect(null); setShoeRect(null);
    } else if (region==='lower') {
      setLowerRect(prev=>smoothRect(prev, lower));
      setUpperRect(null); setFullRect(null); setShoeRect(null);
    } else if (region==='full') {
      setFullRect(prev=>smoothRect(prev, full));
      setUpperRect(null); setLowerRect(null); setShoeRect(null);
    } else if (region==='footwear') {
      setShoeRect(prev=>shoe ? smoothRect(prev, shoe) : null);
      setUpperRect(null); setLowerRect(null); setFullRect(null);
    }
  }
  ...
  function calcMeasurements(lms: PoseLandmark[], worldLms: PoseLandmark[], cw: number, ch: number) {
    ...
    const fallbackSize=sizeFromRatio(ratio,product.category,legRatio);
    const body = {
      shoulderWidthIn: clamp(Math.round(shoulderIn),13,24),
      chestIn: clamp(Math.round(chestIn),28,60),
      torsoHeightIn: Math.round(torPx/ch*28),
      hipWidthIn: clamp(Math.round(hipIn),28,60),
      inseamIn: clamp(Math.round(inseamIn),24,38),
    };
    const fit = getProductFitRecommendation(product, body, fallbackSize);
    setMeasurements({
      ...body,
      recommendedSize:fit.size, confidence, estimatedDistanceM, calibrated,
      fitSummary: fit.summary,
      fitMethod: fit.method,
    });
    if (savedProfile.current) { savedProfile.current.pastFits[product.id]=fit.size; saveProfile(savedProfile.current); }
  }
            ...
            <div className="bg-black/90 text-white rounded-xl p-4 backdrop-blur-sm">
              ...
              <div className="p-3 bg-gradient-to-r from-emerald-900/50 to-green-900/50 border border-emerald-700/40 rounded-lg mb-3">
                ...
                <div className="text-xs text-gray-400 mt-1">
                  {measurements.fitSummary ?? (measurements.calibrated?'Height-calibrated • US standard chart':'Ratio-based estimate • US standard chart')}
                </div>
              </div>
              <AnimatePresence>
                {showMeasurements&&(
                  <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden">
                    <div className="mb-3 p-2 bg-white/5 rounded-lg text-xs space-y-1">
                      ...
                      <div className="flex justify-between"><span className="text-gray-500">Method</span>
                        <span className="text-violet-400">{measurements.fitMethod ?? (userHeightCm?'Height+Pixels':'World Landmarks')}</span></div>
                    </div>
